...
// Strategy:
//   - HTML (navigation): network-first. Always fetch the freshest shell so
//     CSS/JS ?v= bumps appear on ONE reload; fall back to cache only offline.
...
const CACHE_NAME = 'odysseus-v405-modals-16px';
...
self.addEventListener('fetch', (e) => {
  ...
  if (e.request.mode === 'navigate' && url.pathname === '/') {
    ...
    // Was stale-while-revalidate, which served a stale shell first and made
    // edits take two reloads / not appear when the SW held an old copy.
    e.respondWith(
